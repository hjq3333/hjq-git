create database if not exists tms01_ads;
...
create external table ads_trans_order_stats(
 ...
 `dispatch_order_amount` decimal(16,2) COMMENT '发单金额'
    ...
    location '/warehouse/tms01/ads/ads_trans_order_stats';
...
from (select 1                 recent_days,
      ...
      from tms01_dws.dws_trans_org_receive_1d
      where dt = '20250712') receive_1d
         full outer join
     (select 1            recent_days,
             order_count  dispatch_order_count,
             order_amount dispatch_order_amount
      from tms01_dws.dws_trans_dispatch_1d
      where dt = '20250712') dispatch_1d
...
from (select recent_days,
             sum(order_count)  receive_order_count,
             sum(order_amount) receive_order_amount
      from tms01_dws.dws_trans_org_receive_nd
      where dt = '20250712'
     ...
     (select recent_days,
             order_count  dispatch_order_count,
             order_amount dispatch_order_amount
      from tms01_dws.dws_trans_dispatch_nd
      where dt = '20250712') dispatch_nd
...
create external table ads_trans_stats(
   ...
   `trans_finish_dur_sec` bigint COMMENT ' 完成运输时长，单位：秒'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_trans_stats';
...
from tms01_dws.dws_trans_org_truck_model_type_trans_finish_1d
where dt = '20250712'
union
select '20250712'               dt,
       recent_days,
       ...
       sum(trans_finish_distance) trans_finish_distance,
       sum(trans_finish_dur_sec)  trans_finish_dur_sec
from tms01_dws.dws_trans_shift_trans_finish_nd
where dt = '20250712'
...
create external table ads_trans_order_stats_td(
  ...
  `bounding_order_amount` decimal(16,2) COMMENT '运输中运单金额'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_trans_order_stats_td';
...
from (select dt,
             order_count,
             order_amount
      from tms01_dws.dws_trans_dispatch_td
      where dt = '20250712'
      union
      select dt,
             order_count * (-1),
             order_amount * (-1)
      from tms01_dws.dws_trans_bound_finish_td
      where dt = '20250712') new
group by dt;
...
create external table ads_order_stats(
   ...
   `order_amount` decimal(16,2) COMMENT '下单金额'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_order_stats';
...
from tms01_dws.dws_trade_org_cargo_type_order_1d
where dt = '20250712'
union
select '20250712'      dt,
       recent_days,
       sum(order_count)  order_count,
       sum(order_amount) order_amount
from tms01_dws.dws_trade_org_cargo_type_order_nd
where dt = '20250712'
group by recent_days;
...
create external table ads_order_cargo_type_stats(
   ...
   `order_amount` decimal(16,2) COMMENT '下单金额'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_order_cargo_type_stats';
...
create external table ads_city_stats(
   ...
   `avg_trans_finish_dur_sec` bigint COMMENT '平均每次运输时长，单位：秒'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_city_stats';
     ...
     (select '20250712'                                         dt,
      ...
      from (select if(org_level = 1, city_for_level1.id, city_for_level2.id)     city_id,
            ...
            from (select org_id,
                         ...
                         trans_finish_dur_sec
                  from tms01_dws.dws_trans_org_truck_model_type_trans_finish_1d
                  where dt = '20250712') trans_origin
                     left join
                 (select id,
                         org_level,
                         region_id
                  from tms01_dim.dim_organ_full
                  where dt = '20250712') organ
                 on org_id = organ.id
                     left join
                 (select id,
                         name,
                         parent_id
                  from tms01_dim.dim_region_full
...
create external table ads_org_stats(
   ...
   `avg_trans_finish_dur_sec` bigint COMMENT '平均每次运输时长，单位：秒'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_org_stats';
...
create external table ads_shift_stats(
  ...
  `trans_finish_order_count` bigint COMMENT '运输完成运单数'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_shift_stats';
...
create external table ads_line_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_line_stats';
...
create external table ads_driver_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_driver_stats';
...
create external table ads_truck_stats(
   ...
   `avg_trans_finish_dur_sec` bigint COMMENT '平均每次运输时长，单位：秒'
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_truck_stats';
...
create external table ads_express_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_express_stats';
...
from (select '20250712'     dt,
      ...
      from tms01_dws.dws_trans_org_deliver_suc_1d
      where dt = '20250712') deliver_1d
         full outer join
     (select '20250712'    dt,
             1               recent_days,
             sum(sort_count) sort_count
      from tms01_dws.dws_trans_org_sort_1d
      where dt = '20250712') sort_1d
...
from (select '20250712'     dt,
             recent_days,
             sum(order_count) deliver_suc_count
      from tms01_dws.dws_trans_org_deliver_suc_nd
      where dt = '20250712'
     ...
     (select '20250712'    dt,
             recent_days,
             sum(sort_count) sort_count
      from tms01_dws.dws_trans_org_sort_nd
      where dt = '20250712'
...
create external table ads_express_province_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_express_province_stats';
...
create external table ads_express_city_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_express_city_stats';
...
create external table ads_express_org_stats(
    ...
    row format delimited fields terminated by '\t'
    location '/warehouse/tms01/ads/ads_express_org_stats';
