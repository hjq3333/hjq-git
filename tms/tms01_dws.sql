create database if not exists tms0101_dws;
...
create external table dws_trade_org_cargo_type_order_1d(
    ...
    stored as orc
    location '/warehouse/tms01/dws/dws_trade_org_cargo_type_order_1d'
...
from (select org_id,
      ...
      from (select order_id,
            ...
            from (select order_id,
                         ...
                         amount,
                         dt
                  from tms01_dwd.dwd_trade_order_detail_inc) detail
            group by order_id,
                     cargo_type,
                     cargo_type_name,
                     sender_district_id,
...
create external table dws_trans_org_receive_1d(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_receive_1d/'
    tblproperties ('orc.compress'='snappy');
...
from (select order_id,
      ...
      from (select order_id,
                   ...
                   sender_district_id,
                   dt
            from tms01_dwd.dwd_trans_receive_detail_inc) detail
               left join
           (select id org_id,
                   org_name,
                   region_id
...
create external table dws_trans_dispatch_1d(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_dispatch_1d/'
    tblproperties('orc.compress'='snappy');
...
from (select order_id,
             dt,
             max(amount) distinct_amount
      from tms01_dwd.dwd_trans_dispatch_detail_inc
      group by order_id,
               dt) distinct_info
group by dt;
...
create external table dws_trans_org_truck_model_type_trans_finish_1d(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_truck_model_type_trans_finish_1d/'
    tblproperties('orc.compress'='snappy');
...
from (select id,
             ...
             finish_dur_sec,
             dt
      from tms01_dwd.dwd_trans_trans_finish_inc) trans_finish
         left join
     (select id,
             truck_model_type,
             truck_model_type_name
      from tms01_dim.dim_truck_full
...
create external table dws_trans_org_deliver_suc_1d(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_deliver_suc_1d/'
    tblproperties('orc.compress'='snappy');
...
from (select order_id,
             ...
             dt
      from tms01_dwd.dwd_trans_deliver_suc_detail_inc
...
create external table dws_trans_org_sort_1d(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_sort_1d/'
    tblproperties('orc.compress'='snappy');
...
from (select org_id,
             count(*) sort_count,
             dt
      from tms01_dwd.dwd_bound_sort_inc
     ...
     (select id,
             ...
             parent_id
      from tms01_dim.dim_region_full
      where dt = '20250712') city_for_level1
     ...
     (select id,
             ...
             parent_id
      from tms01_dim.dim_region_full
      where dt = '20250712') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from tms01_dim.dim_region_full
      where dt = '20250712') province_for_level2
...
create external table dws_trade_org_cargo_type_order_nd(
    ...
    partitioned by(`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trade_org_cargo_type_order_nd'
    tblproperties('orc.compress' = 'snappy');
...
create external table dws_trans_org_receive_nd(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_receive_nd/'
    tblproperties ('orc.compress'='snappy');
...
create external table dws_trans_dispatch_nd(
  ...
  `order_amount` decimal(16,2) comment '发单总金额'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_dispatch_nd/'
    tblproperties('orc.compress'='snappy');
...
create external table dws_trans_shift_trans_finish_nd(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_shift_trans_finish_nd/'
    tblproperties('orc.compress'='snappy');
...
from (select recent_days,
      ...
      from tms01_dwd.dwd_trans_trans_finish_inc lateral view
          explode(array(7, 30)) tmp as recent_days
      group by recent_days,
               shift_id,
               line_id,
...
create external table dws_trans_org_deliver_suc_nd(
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_deliver_suc_nd/'
    tblproperties('orc.compress'='snappy');
...
from dws_trans_org_deliver_suc_1d lateral view
    explode(array(7, 30)) tmp as recent_days
where dt >= date_add('20250712', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
...
create external table dws_trans_org_sort_nd(
   ...
   `sort_count` bigint comment '分拣次数'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_org_sort_nd/'
    tblproperties('orc.compress'='snappy');
...
create external table dws_trans_dispatch_td(
  ...
  `order_amount` decimal(16,2) comment '发单金额'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/warehouse/tms01/dws/dws_trans_dispatch_td'
    tblproperties('orc.compress'='snappy');
...
create external table dws_trans_bound_finish_td(
   ...
   `order_amount` decimal(16,2) comment '发单金额'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location 'warehouse/tms01/dws/dws_trans_bound_finish_td'
    tblproperties('orc.compress'='snappy');
...
from (select order_id,
      ...
      from tms01_dwd.dwd_trans_bound_finish_detail_inc
