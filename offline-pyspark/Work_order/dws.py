...
wireless_daily_data = spark.table("work_order.dwd_wireless_entry_detail").filter(
...
# 创建HDFS目录
create_hdfs_dir("/warehouse/work_order/dws/dws_pc_entry_daily")

# 创建表结构
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS work_order.dws_pc_entry_daily (
    page_type STRING COMMENT '页面类型',
    stat_date DATE COMMENT '统计日期',
    visitor_count BIGINT COMMENT '当日访客数',
    order_buyer_count BIGINT COMMENT '当日下单买家数'
...
PARTITIONED BY (dt STRING COMMENT '分区日期（与stat_date一致）')
STORED AS PARQUET
LOCATION '/warehouse/work_order/dws/dws_pc_entry_daily'
TBLPROPERTIES ('parquet.compression' = 'snappy');
""")

# 从DWD层读取PC端数据，按页面类型聚合
pc_daily_data = spark.table("work_order.dwd_pc_entry_detail").filter(
    F.col("dt") == process_date
).groupBy("page_type") \
    .agg(
    F.countDistinct("visitor_id").alias("visitor_count"),  # 去重访客数
...
# 1. 无线端日度数据
wireless_day_data = spark.table("work_order.dws_wireless_entry_daily").filter(
    F.col("dt") == process_date
).select(
    "page_type",
...
# 2. 无线端7天数据
wireless_seven_day_data = spark.table("work_order.dws_wireless_entry_daily").filter(
...
# 3. 无线端30天数据
wireless_thirty_day_data = spark.table("work_order.dws_wireless_entry_daily").filter(
...
# 创建HDFS目录
create_hdfs_dir("/warehouse/work_order/dws/dws_pc_entry_multi_period")

# 创建表结构
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS work_order.dws_pc_entry_multi_period (
    page_type STRING COMMENT '页面类型',
    stat_period STRING COMMENT '统计周期（day/7day/30day）',
    end_date DATE COMMENT '周期结束日期',
    total_visitor_count BIGINT COMMENT '周期内总访客数',
...
PARTITIONED BY (dt STRING COMMENT '分区日期（与end_date一致）')
STORED AS PARQUET
LOCATION '/warehouse/work_order/dws/dws_pc_entry_multi_period'
TBLPROPERTIES ('parquet.compression' = 'snappy');
""")

# 1. PC端日度数据
pc_day_data = spark.table("work_order.dws_pc_entry_daily").filter(
    F.col("dt") == process_date
).select(
    "page_type",
    F.lit("day").alias("stat_period"),
...
# 2. PC端7天数据
pc_seven_day_data = spark.table("work_order.dws_pc_entry_daily").filter(
    F.col("dt").between(start_date_7d, process_date)
).groupBy("page_type") \
    .agg(
    F.sum("visitor_count").alias("total_visitor_count"),
...
# 3. PC端30天数据
pc_thirty_day_data = spark.table("work_order.dws_pc_entry_daily").filter(
    F.col("dt").between(start_date_30d, process_date)
).groupBy("page_type") \
    .agg(
    F.sum("visitor_count").alias("total_visitor_count"),
...
# # ====================== DWS层：店铺页各种页排行每日指标汇总（简化版） ======================
# # 创建DWS表
# spark.sql("""
# CREATE EXTERNAL  TABLE if  NOT EXISTS work_order.dws_shop_page_daily_summary (
...
# STORED AS PARQUET
# LOCATION '/warehouse/work_order/dws/dws_shop_page_daily_summary'
# TBLPROPERTIES ('parquet.compression' = 'snappy');
# """)
#
# process_date = '20250704'
#
# # 从DWD层计算DWS指标（只保留需要的字段）
# dws_data = spark.table("work_order.dwd_shop_page_visit_detail").filter(
#     F.col("dt") == process_date
...
#     .parquet(f"/warehouse/work_order/dws/dws_shop_page_daily_summary/dt={process_date}")
#
# repair_hive_table("dws_shop_page_daily_summary")
#
# # 验证DWS数据
# print("DWS层数据预览：")
# dws_data.show()
# print("DWS层字段类型：")
# dws_data.printSchema()
#
#
# # ====================== DWS层：路径聚合（追加模式） ======================
# # 1. 首次运行创建表结构
# spark.sql("""
# CREATE EXTERNAL TABLE IF NOT EXISTS work_order.dws_instore_path_summary (
...
# STORED AS PARQUET
# LOCATION '/warehouse/work_order/dws/dws_instore_path_summary'
# TBLPROPERTIES ('parquet.compression' = 'snappy');
# """)
#
# create_hdfs_dir("/warehouse/work_order/dws/dws_instore_path_summary")
#
# process_date = '20250702'
#
# # 2. 从DWD层加载当日数据聚合
# source_pages = spark.table("work_order.dwd_instore_path").filter(F.col("dt") == process_date) \
#     .select(
#     F.col("visitor_id"),
...
# )
#
# target_pages = spark.table("work_order.dwd_instore_path").filter(F.col("dt") == process_date) \
#     .select(
#     F.col("visitor_id"),
...
#
# # 合并并去重当日页面访问记录
# all_pages = source_pages.unionByName(target_pages).dropDuplicates(
#     ["visitor_id", "page_id", "visit_sequence"]
# )
#
# # 按访客ID聚合当日路径
# dws_data = all_pages.groupBy("visitor_id", "jump_date") \
#     .agg(
#     F.collect_list(
#         F.struct(
#             "page_id",
...
# dws_data.write.mode("overwrite") \
#     .parquet(f"/warehouse/work_order/dws/dws_instore_path_summary/dt={process_date}")
#
# repair_hive_table("dws_instore_path_summary")
#
# # 验证DWS数据
# print(f"DWS层{process_date}新增路径数：{dws_data.count()}条")
# print(f"DWS层历史总路径数：{spark.table('work_order.dws_instore_path_summary').count()}条")
#
#
#
# # ====================== DWS层：pc来源页面指标聚合 ======================
# # 1. 首次运行创建表结构（调整分组字段相关注释）
# spark.sql("""
# CREATE EXTERNAL TABLE IF NOT EXISTS work_order.dws_pc_source_summary (
#     source_page_name STRING COMMENT '来源页面名称（分组依据）',
#     visitor_count BIGINT COMMENT '独立访客数',
#     visit_count BIGINT COMMENT '总访问次数',
#     data_date DATE COMMENT '数据日期（分组依据）',
#     -- 保留URL和ID的聚合信息，便于参考
#     source_page_ids STRING COMMENT '该名称对应的所有页面ID（逗号分隔）',
#     source_page_urls STRING COMMENT '该名称对应的所有页面URL（逗号分隔）'
# )
# PARTITIONED BY (dt STRING COMMENT '分区日期')
# STORED AS PARQUET
# LOCATION '/warehouse/work_order/dws/dws_pc_source_summary'
# TBLPROPERTIES ('parquet.compression' = 'snappy');
# """)
#
#
# create_hdfs_dir("/warehouse/work_order/dws/dws_pc_source_summary")
#
# process_date = '20250701'
# # 2. 从DWD聚合（核心修改：仅按页面名称和日期分组）
# dws_data = spark.table("work_order.dwd_pc_traffic_entry") \
#     .filter(F.col("dt") == process_date) \
#     .groupBy("source_page_name", "data_date") \
#     .agg(
#     F.countDistinct("visitor_id").alias("visitor_count"),
#     F.count("*").alias("visit_count"),
#     # 聚合同一名称对应的所有ID和URL，便于追溯
#     F.concat_ws(",", F.collect_set("source_page_id")).alias("source_page_ids"),
#     F.concat_ws(";", F.collect_set("source_page_url")).alias("source_page_urls")
# ) \
#     .withColumn("dt", F.lit(process_date))
#
#
# # 3. 写入当日分区
# dws_data.write.mode("overwrite") \
#     .parquet(f"/warehouse/work_order/dws/dws_pc_source_summary/dt={process_date}")
#
# repair_hive_table("dws_pc_source_summary")
# print(f"DWS层{process_date}处理完成，按页面名称聚合后数据量：{dws_data.count()}条")