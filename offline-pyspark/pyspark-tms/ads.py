...
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__trans_order_stats
(
...
LOCATION '/warehouse/tms_pyspark/ads/ads_trans_order_stats'
""")

# PySpark implementation
receive_1d = spark.table("tms_spark_dws.dws_trans_org_receive_1d") \
...
dispatch_1d = spark.table("tms_spark_dws.dws_trans_dispatch_1d") \
    .filter(col("dt") == "20250713") \
    .select(
    lit(1).alias("recent_days"),
...
receive_nd = spark.table("tms_spark_dws.dws_trans_org_receive_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("recent_days") \
    .agg(
    sum("order_count").alias("receive_order_count"),
...
dispatch_nd = spark.table("tms_spark_dws.dws_trans_dispatch_nd") \
    .filter(col("dt") == "20250713") \
    .select(
    "recent_days",
    col("order_count").alias("dispatch_order_count"),
...
# 2. ads_trans_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_trans_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__trans_stats
(
    `dt`                    string COMMENT '统计日期',
    `recent_days`           tinyint COMMENT '最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_trans_stats'
""")

# PySpark implementation
trans_1d = spark.table("tms_spark_dws.dws_trans_org_truck_model_type_trans_finish_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy() \
    .agg(
    lit(1).alias("recent_days"),
...
trans_nd = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("recent_days") \
    .agg(
    sum("trans_finish_count").alias("trans_finish_count"),
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_trans_order_stats_td")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__trans_order_stats_td
(
    `dt`                    string COMMENT '统计日期',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_trans_order_stats_td'
""")

# PySpark implementation
dispatch_td = spark.table("tms_spark_dws.dws_trans_dispatch_td") \
    .filter(col("dt") == "20250713") \
    .select(
    "dt",
    "order_count",
...
bound_finish_td = spark.table("tms_spark_dws.dws_trans_bound_finish_td") \
    .filter(col("dt") == "20250713") \
    .select(
    "dt",
...
# 4. ads_order_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_order_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__order_stats
(
    `dt`           string COMMENT '统计日期',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_order_stats'
""")

# PySpark implementation
order_1d = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy() \
    .agg(
    lit(1).alias("recent_days"),
...
order_nd = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("recent_days") \
    .agg(
    sum("order_count").alias("order_count"),
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_order_cargo_type_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__order_cargo_type_stats
(
    `dt`              string COMMENT '统计日期',
    `recent_days`     tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_order_cargo_type_stats'
""")

# PySpark implementation
cargo_1d = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_1d") \
    .filter(col("dt") == "20250713") \
...
cargo_nd = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_nd") \
    .filter(col("dt") == "20250713") \
...
# 6. ads_city_stats
# 6. ads_city_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_city_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__city_stats
(
    `dt`                        string COMMENT '统计日期',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_city_stats'
""")

# PySpark implementation for 1 day
city_order_1d = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_1d") \
    .filter(col("dt") == "20250713") \
...
dim_organ_full = spark.table("tms_spark_dim.dim_organ_full").filter(col("dt") == "20250713").select("id", "org_level", "region_id").alias("organ")
dim_region_full = spark.table("tms_spark_dim.dim_region_full").filter(col("dt") == "20250713").select("id", "name", "parent_id").alias("region")
dim_region_full2 = spark.table("tms_spark_dim.dim_region_full").filter(col("dt") == "20250713").select(col("id").alias("id2"), col("name").alias("name2")).alias("region2")

city_trans_1d = spark.table("tms_spark_dws.dws_trans_org_truck_model_type_trans_finish_1d") \
    .filter(col("dt") == "20250713") \
    .alias("trans") \
...
city_order_nd = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_nd") \
    .filter(col("dt") == "20250713") \
...
city_trans_nd = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name", "recent_days") \
    .agg(
    sum("trans_finish_count").alias("trans_finish_count"),
...
# 7. ads_org_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_org_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__org_stats
(
    `dt`                        string COMMENT '统计日期',
    `recent_days`               tinyint COMMENT '最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_org_stats'
""")

# PySpark implementation for 1 day
org_order_1d = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_1d") \
    .filter(col("dt") == "20250713") \
...
org_trans_1d = spark.table("tms_spark_dws.dws_trans_org_truck_model_type_trans_finish_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name") \
    .agg(
    lit(1).alias("recent_days"),
...
# PySpark implementation for n days
org_order_nd = spark.table("tms_spark_dws.dws_trade_org_cargo_type_order_nd") \
    .filter(col("dt") == "20250713") \
...
org_trans_nd = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name", "recent_days") \
    .agg(
    sum("trans_finish_count").alias("trans_finish_count"),
...
# 8. ads_shift_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_shift_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__shift_stats
(
    `dt`                       string COMMENT '统计日期',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_shift_stats'
""")

# PySpark implementation
shift_stats = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
    .select(
    lit("20250713").alias("dt"),
    col("recent_days"),
...
# 9. ads_line_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_line_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__line_stats
(
    `dt`                       string COMMENT '统计日期',
    `recent_days`              tinyint COMMENT '最近天数,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_line_stats'
""")

# PySpark implementation
line_stats = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
...
# 10. ads_driver_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_driver_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__driver_stats
(
    `dt`                        string COMMENT '统计日期',
    `recent_days`               tinyint COMMENT '最近天数,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_driver_stats'
""")

# PySpark implementation
single_driver = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
...
dual_driver = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
...
# 11. ads_truck_stats
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_truck_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__truck_stats
(
    `dt`                        string COMMENT '统计日期',
    `recent_days`               tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_truck_stats'
""")

# PySpark implementation
truck_stats = spark.table("tms_spark_dws.dws_trans_shift_trans_finish_nd") \
    .filter(col("dt") == "20250713") \
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_express_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__express_stats
(
    `dt`                string COMMENT '统计日期',
    `recent_days`       tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_express_stats'
""")

# PySpark implementation for 1 day
deliver_1d = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy() \
    .agg(
    lit(1).alias("recent_days"),
...
sort_1d = spark.table("tms_spark_dws.dws_trans_org_sort_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy() \
    .agg(
    lit(1).alias("recent_days"),
...
# PySpark implementation for n days
deliver_nd = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("recent_days") \
    .agg(
    sum("order_count").alias("deliver_suc_count")
...
sort_nd = spark.table("tms_spark_dws.dws_trans_org_sort_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("recent_days") \
    .agg(
    sum("sort_count").alias("sort_count")
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_express_province_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__express_province_stats
(
    `dt`                   string COMMENT '统计日期',
    `recent_days`          tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_express_province_stats'
""")

# PySpark implementation for 1 day
province_deliver_1d = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_1d") \
    .filter(col("dt") == "20250713") \
...
province_sort_1d = spark.table("tms_spark_dws.dws_trans_org_sort_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("province_id", "province_name") \
    .agg(
    lit(1).alias("recent_days"),
...
province_receive_1d = spark.table("tms_spark_dws.dws_trans_org_receive_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("province_id", "province_name") \
    .agg(
    lit(1).alias("recent_days"),
...
# PySpark implementation for n days
province_deliver_nd = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_nd") \
    .filter(col("dt") == "20250713") \
...
province_sort_nd = spark.table("tms_spark_dws.dws_trans_org_sort_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("province_id", "province_name", "recent_days") \
    .agg(
    sum("sort_count").alias("sort_count")
...
province_receive_nd = spark.table("tms_spark_dws.dws_trans_org_receive_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("province_id", "province_name", "recent_days") \
    .agg(
    sum("order_count").alias("receive_order_count"),
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_express_city_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__express_city_stats
(
    `dt`                   string COMMENT '统计日期',
    `recent_days`          tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
    `city_id`              bigint COMMENT '城市ID',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_express_city_stats'
""")

# PySpark implementation for 1 day
city_deliver_1d = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name") \
    .agg(
    lit(1).alias("recent_days"),
...
city_sort_1d = spark.table("tms_spark_dws.dws_trans_org_sort_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name") \
    .agg(
    lit(1).alias("recent_days"),
...
city_receive_1d = spark.table("tms_spark_dws.dws_trans_org_receive_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name") \
    .agg(
    lit(1).alias("recent_days"),
...
# PySpark implementation for n days
city_deliver_nd = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name", "recent_days") \
    .agg(
    sum("order_count").alias("deliver_suc_count")
...
city_sort_nd = spark.table("tms_spark_dws.dws_trans_org_sort_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name", "recent_days") \
    .agg(
    sum("sort_count").alias("sort_count")
...
city_receive_nd = spark.table("tms_spark_dws.dws_trans_org_receive_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("city_id", "city_name", "recent_days") \
    .agg(
    sum("order_count").alias("receive_order_count"),
...
spark.sql("DROP TABLE IF EXISTS tms_spark_ads.ads_express_org_stats")
spark.sql("""
CREATE EXTERNAL TABLE IF NOT EXISTS  tms_spark_ads.ads__express_org_stats
(
    `dt`                   string COMMENT '统计日期',
    `recent_days`          tinyint COMMENT '最近天数,1:最近天数,1:最近1天,7:最近7天,30:最近30天',
    `org_id`               bigint COMMENT '机构ID',
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/warehouse/tms_pyspark/ads/ads_express_org_stats'
""")

# PySpark implementation for 1 day
org_deliver_1d = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name") \
    .agg(
    lit(1).alias("recent_days"),
...
org_sort_1d = spark.table("tms_spark_dws.dws_trans_org_sort_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name") \
    .agg(
    lit(1).alias("recent_days"),
...
org_receive_1d = spark.table("tms_spark_dws.dws_trans_org_receive_1d") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name") \
    .agg(
    lit(1).alias("recent_days"),
...
# PySpark implementation for n days
org_deliver_nd = spark.table("tms_spark_dws.dws_trans_org_deliver_suc_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name", "recent_days") \
    .agg(
    sum("order_count").alias("deliver_suc_count")
...
org_sort_nd = spark.table("tms_spark_dws.dws_trans_org_sort_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name", "recent_days") \
    .agg(
    sum("sort_count").alias("sort_count")
...
org_receive_nd = spark.table("tms_spark_dws.dws_trans_org_receive_nd") \
    .filter(col("dt") == "20250713") \
    .groupBy("org_id", "org_name", "recent_days") \
    .agg(
    sum("order_count").alias("receive_order_count"),
