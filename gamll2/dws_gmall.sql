...
create database if not exists dws
    LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/gmall/dws/dws1';
...
(
    ...
    STORED AS ORC
    LOCATION '/bigdata_warehouse/gmall/dws/dws1/dws_trade_user_sku_order_1d'
...
from (select dt,
     ...
     (select id,
      ...
      where dt = '${latest_dim_date}' -- 使用变量
     ) sku
     on cast(od.sku_id as string) = sku.id;
...
set hive.vectorized.execution.enabled = false;
insert overwrite table dws.dws_trade_user_sku_order_1d partition (dt = '2025-06-30')
select user_id,
       id,
       sku_name,
       category1_id,
...
(
    ...
    PARTITIONED BY (`dt` STRING)
    STORED AS ORC
    LOCATION '/bigdata_warehouse/gmall/dws/dws1/dws_trade_user_order_1d'
    TBLPROPERTIES ('orc.compress' = 'snappy');
...
insert overwrite table dws.dws_trade_user_order_1d partition (dt = '2025-06-30')
select user_id,
       count(distinct (order_id)),
       sum(sku_num),
       sum(split_original_amount),
...
(
    ...
    PARTITIONED BY (`dt` STRING)
    STORED AS ORC
    LOCATION '/bigdata_warehouse/gmall/dws/dws1/dws_trade_user_cart_add_1d'
    TBLPROPERTIES ('orc.compress' = 'snappy');
...
insert overwrite table dws.dws_trade_user_cart_add_1d partition (dt = '2025-06-30')
select user_id,
       count(*),
       sum(sku_num)
from dwd.dwd_trade_cart_add_inc
...
(
    ...
    PARTITIONED BY (`dt` STRING)
    STORED AS ORC
    LOCATION '/bigdata_warehouse/gmall/dws/dws1/dws_trade_province_order_1d'
    TBLPROPERTIES ('orc.compress' = 'snappy');
...
insert overwrite table dws.dws_trade_province_order_1d partition (dt = '2025-06-30')
select province_id,
       province_name,
       area_code,
       iso_code,
...
(
    ...
    PARTITIONED BY (dt STRING)
    STORED AS ORC
    LOCATION '/bigdata_warehouse/gmall/dws/dws1/dws_interaction_sku_favor_add_1d'
    TBLPROPERTIES ('orc.compress' = 'snappy');
...
insert overwrite table dws.dws_interaction_sku_favor_add_1d partition (dt = '2025-06-30')
select sku_id,
       sku_name,
       category1_id,
       category1_name,
...
CREATE EXTERNAL TABLE dws.dws_traffic_session_page_view_1d
(
    ...
    PARTITIONED BY (`dt` STRING)
    STORED AS ORC
    LOCATION '/warehouse/gmall/dws/dws1/dws_traffic_session_page_view_1d'
    TBLPROPERTIES ('orc.compress' = 'snappy');


insert overwrite table dws.dws_traffic_session_page_view_1d partition (dt = '2025-06-30')
select session_id,
       mid_id,
       brand,
       model,
...
select *
from dws.dws_traffic_session_page_view_1d;
...
select nvl(old.user_id, new.user_id)                                      user_id,
       if(new.user_id is null, old.login_date_last, '20250630')           login_date_last,
       if(old.login_date_first is null, '20250630', old.login_date_first) login_date_first,
...
from (select user_id,
      ...
      where dt = date_add('20250630', -1)) old
         full outer join
     (select user_id,
...
insert overwrite table dws.dws_traffic_page_visitor_page_view_1d partition (dt = '20250630')
select mid_id,
       brand,
       model,
       operate_system,
...
group by user_id, dt;
--② 每日装载
insert overwrite table dws.dws_trade_user_payment_1d partition (dt = '2025-06-30')
select user_id,
       count(distinct (order_id)),
       sum(sku_num),
       sum(split_payment_amount)
...
insert overwrite table dws.dws_trade_user_order_td partition (dt = '2025-06-30')
select user_id,
...
select nvl(old.user_id, new.user_id),
       if(old.user_id is not null, old.order_date_first, '2025-06-30'),
       if(new.user_id is not null, '2025-06-30', old.order_date_last),
...
from (select user_id,
      ...
      where dt = date_add('2025-06-30', -1)) old
         full outer join
     (select user_id,
             order_count_1d,
             order_num_1d,
...
from (select user_id,
             ...
             total_amount_td
      from dws.dws_trade_user_order_td
      where dt = date_add('2025-06-30', -1)
      union all
      select user_id,
             '2025-06-30' order_date_first,
             '2025-06-30' order_date_last,
             order_count_1d,
             order_num_1d,
             order_original_amount_1d,
             activity_reduce_amount_1d,
...
insert overwrite table dws.dws_trade_user_sku_order_nd partition (dt = '2025-06-30')
select user_id,
       sku_id,
       sku_name,
       category1_id,
...
insert overwrite table dws.dws_trade_province_order_nd partition (dt = '2025-06-30')
select province_id,
       province_name,
       area_code,
       iso_code,
...
select *
from dws.dws_trade_province_order_nd;
...
from (select dt,
      ...
      group by dt, user_id, coupon_id) t1
         left join
     (select id,
             coupon_name,
             coupon_type_code,
      ...
      where dt = '2025-06-30') t2
     on t1.coupon_id = t2.id;
--② 每日装载
insert overwrite table dws.dws_tool_user_coupon_coupon_used_1d partition (dt = '2025-06-30')
select user_id,
       coupon_id,
       coupon_name,
       coupon_type_code,
